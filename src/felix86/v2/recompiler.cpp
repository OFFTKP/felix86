...
// This comes with the slight downside that every place that links to a block needs to set t5 to its own address
...
void Recompiler::invalidateAt(ThreadState* state, u8* linked_block, u8* invalid_block) {
    ...
    if (linked_block) {
        ...
        // To confirm this, we can check that it truly is an AUIPC(t5, 0):
        u32 instruction = 0;
        Assembler tas((u8*)&instruction, 4);
        tas.AUIPC(t5, 0);
...
u64 Recompiler::compileSequence(bool mode32, u64 rip) {
    ...
    while (compiling) {
        ...
        if (is_x87) {
            ...
            if (!is_save_restore && local_x87_state != x87State::x87) {
                switchToX87();
            }
        }
...
biscuit::GPR Recompiler::getGPRSigned(x86_ref_e ref, x86_size_e size, bool dont_sext32) {
    ...
    case X86_SIZE_BYTE: {
        ...
        sext(gpr8, gpr, X86_SIZE_BYTE);
        return gpr8;
    }
    case X86_SIZE_BYTE_HIGH: {
        biscuit::GPR gpr8 = scratch();
        as.SRLI(gpr8, gpr, 8);
        sext(gpr8, gpr8, X86_SIZE_BYTE);
        return gpr8;
    }
    case X86_SIZE_WORD: {
        biscuit::GPR gpr16 = scratch();
        sext(gpr16, gpr, X86_SIZE_WORD);
        return gpr16;
    }
    case X86_SIZE_DWORD: {
        if (!dont_sext32) {
...
biscuit::FPR Recompiler::getElementFPR(ZydisDecodedOperand* operand, x86_size_e size, int element) {
    ...
    switch (size) {
    case X86_SIZE_DWORD: {
        as.FLW(temp, offset, address);
        break;
    }
    case X86_SIZE_QWORD: {
        as.FLD(temp, offset, address);
        break;
    }
    default: {
        UNREACHABLE();
...
biscuit::GPR Recompiler::lea(const ZydisDecodedOperand* operand, bool use_temp, biscuit::GPR forced_dst, i64* offset) {
    ...
    if (current_instruction->attributes & ZYDIS_ATTRIB_HAS_ADDRESSSIZE) {
                    ...
                    auto scan_landing_block = [&](u64 rip_ahead) {
                        ...
                        for (size_t i = 0; i < scan_ahead_count; i++) {
                            ...
                            if (is_jump || is_illegal || is_hlt || is_int3) {
                                ...
                                if (mnemonic == ZYDIS_MNEMONIC_JMP && !jump_to_self) {
                                    operands_ahead = operands_ahead_storage;
                                    decode(rip_ahead, instruction_ahead, operands_ahead);
                                }

                                if (mnemonic == ZYDIS_MNEMONIC_JMP && operands_ahead[0].type == ZYDIS_OPERAND_TYPE_IMMEDIATE) {
...
void Recompiler::readMemorySigned(biscuit::GPR dest, biscuit::GPR address, i64 offset, x86_size_e size) {
    switch (size) {
    case X86_SIZE_BYTE: {
        as.LB(dest, offset, address);
        break;
    }
    case X86_SIZE_WORD: {
        as.LH(dest, offset, address);
        break;
    }
    case X86_SIZE_DWORD: {
        as.LW(dest, offset, address);
        break;
    }
    case X86_SIZE_QWORD: {
        as.LD(dest, offset, address);
...
biscuit::FPR Recompiler::getST(int index, bool dirty, bool convert_to_64_bit) {
    ...
    // written back
    // For example, if we load 2 regs, then access ST2, in memory this ST2
    // will be ST0 (the ST at the top)
    biscuit::GPR address = scratch();
    ...
    x87_reg_cache[index].loaded = true;
    x87_reg_cache[index].dirty = dirty;
