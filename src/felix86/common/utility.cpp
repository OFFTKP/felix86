...
void felix86_fsin(ThreadState* state) {
    ...
    memcpy(&state->ctx.st[TOP(0)], &result, sizeof(double));
...
void felix86_fptan(ThreadState* state) {
    double st0;
    memcpy(&st0, &state->ctx.st[TOP(0)], sizeof(double));
...
void felix86_fpatan(ThreadState* state) {
    double st0, st1;
    memcpy(&st0, &state->ctx.st[TOP(0)], sizeof(double));
    memcpy(&st1, &state->ctx.st[TOP(1)], sizeof(double));
    double result = ::atan2(st1, st0);
    memcpy(&state->ctx.st[TOP(1)], &result, sizeof(double));
}
...
void felix86_fprem(ThreadState* state) {
    const u64 st0 = state->ctx.st[TOP(0)].significand;
    const u64 st1 = state->ctx.st[TOP(1)].significand;
    double st0d, st1d;
    memcpy(&st0d, &st0, 8);
    memcpy(&st1d, &st1, 8);
    ...
    memcpy(&state->ctx.st[TOP(0)], &st0d, 8);
}

void felix86_fxam(ThreadState* state) {
    u64 st0 = state->ctx.st[TOP(0)].significand;
    bool sign = st0 >> 63;
    double st0d;
    memcpy(&st0d, &st0, 8);
...
static u32 aes_subword(u32 word) {
    static u8 table[256] = {0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76, 0xca, 0x82, 0xc9, 0x7d,
    ...
    u8 b0 = table[(word >> 0) & 0xFF];
    u8 b1 = table[(word >> 8) & 0xFF];
    u8 b2 = table[(word >> 16) & 0xFF];
    u8 b3 = table[(word >> 24) & 0xFF];
...
void felix86_aeskeygenassist(u32* dst, u32* src, u32 imm) {
    u32 x1 = *(src + 1);
    u32 x3 = *(src + 3);
    u32 x1_sub = aes_subword(x1);
    u32 x1_sub_rotate = x1_sub >> 8 | x1_sub << 24;
    u32 x3_sub = aes_subword(x3);
    u32 x3_sub_rotate = x3_sub >> 8 | x3_sub << 24;
    *dst = x1_sub;
    *(dst + 1) = x1_sub_rotate ^ imm;
    *(dst + 2) = x3_sub;
