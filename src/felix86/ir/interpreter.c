                                     ...
                                     ir_instruction_t* instruction, x86_thread_state_t* state)
{
    switch (instruction->opcode)
    {
        ...
        {
            xmm_temps[instruction->name].data[0] =
        ...
        case IR_POPCOUNT:
        {
            temps[instruction->name] =
                __builtin_popcountll(temps[instruction->operands.args[0]->name]);
            break;
        }
        case IR_MOV:
        {
        ...
        case IR_GET_GUEST:
        {
            switch (instruction->get_guest.ref)
            {
                ...
                {
                    temps[instruction->name] =
                        state->gprs[instruction->get_guest.ref - X86_REF_RAX];
                    break;
                }
                case X86_REF_CF:
                {
                ...
                case X86_REF_XMM0 ... X86_REF_XMM15:
                {
                    xmm_temps[instruction->name] =
                        state->xmm[instruction->get_guest.ref - X86_REF_XMM0];
                    break;
                }
                default:
                {
