...
IR_HANDLE(error)
{
    ...
    ERROR("Hit error instruction during: %016lx - Opcode: %02x",
          state->current_address - g_base_address, inst->opcode);
}
...
IR_HANDLE(add_reg_rm)
{ // add r16/32/64, rm16/32/64 - 0x03
    x86_size_e size_e = inst->operand_reg.size;
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
...
IR_HANDLE(add_eax_imm)
{ // add ax/eax/rax, imm16/32/64 - 0x05
    x86_size_e size_e = inst->operand_reg.size;
    ir_instruction_t* eax = ir_emit_get_reg(INSTS, &inst->operand_reg);
    ir_instruction_t* imm =
        ir_emit_immediate(INSTS, sext_if_64(inst->operand_imm.immediate.data, size_e));
...
IR_HANDLE(or_eax_imm)
{ // add ax/eax/rax, imm16/32/64 - 0x0D
    x86_size_e size_e = inst->operand_reg.size;
    ir_instruction_t* eax = ir_emit_get_reg(INSTS, &inst->operand_reg);
    ir_instruction_t* imm =
        ir_emit_immediate(INSTS, sext_if_64(inst->operand_imm.immediate.data, size_e));
...
IR_HANDLE(and_rm_reg)
{ // and rm16/32/64, r16/32/64 - 0x21
    x86_size_e size_e = inst->operand_reg.size;
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
...
IR_HANDLE(and_eax_imm)
{ // and ax/eax/rax, imm16/32/64 - 0x25
    x86_size_e size_e = inst->operand_reg.size;
    ir_instruction_t* eax = ir_emit_get_reg(INSTS, &inst->operand_reg);
    ir_instruction_t* imm =
        ir_emit_immediate(INSTS, sext_if_64(inst->operand_imm.immediate.data, size_e));
...
IR_HANDLE(sub_rm_reg)
{ // sub rm16/32/64, r16/32/64 - 0x29
    x86_size_e size_e = inst->operand_reg.size;
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
...
IR_HANDLE(sub_eax_imm)
{ // sub ax/eax/rax, imm16/32/64 - 0x2d
    x86_size_e size_e = inst->operand_reg.size;
    ir_instruction_t* eax = ir_emit_get_reg(INSTS, &inst->operand_reg);
    ir_instruction_t* imm =
        ir_emit_immediate(INSTS, sext_if_64(inst->operand_imm.immediate.data, size_e));
...
IR_HANDLE(xor_reg_rm)
{ // xor r16/32/64, rm16/32/64 - 0x33
    x86_size_e size_e = inst->operand_reg.size;
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
...
IR_HANDLE(xor_eax_imm)
{ // xor ax/eax/rax, imm16/32/64 - 0x35
    x86_size_e size_e = inst->operand_reg.size;
    ir_instruction_t* eax = ir_emit_get_reg(INSTS, &inst->operand_reg);
    ir_instruction_t* imm =
        ir_emit_immediate(INSTS, sext_if_64(inst->operand_imm.immediate.data, size_e));
...
{ // jcc rel8 - 0x70-0x7f
    ...
    ir_block_t* block_true =
        ir_function_get_block(state->function, state->current_block, jump_address_true);
    ir_block_t* block_false =
        ir_function_get_block(state->function, state->current_block, jump_address_false);
...
IR_HANDLE(group1_rm8_imm8)
{ // add/or/adc/sbb/and/sub/xor/cmp rm8, imm8 - 0x80
    ir_emit_group1_imm(INSTS, inst);
}

IR_HANDLE(group1_rm32_imm32)
{ // add/or/adc/sbb/and/sub/xor/cmp rm16/32/64,
  // imm16/32/64 - 0x81
    ir_emit_group1_imm(INSTS, inst);
}

IR_HANDLE(group1_rm32_imm8)
{ // add/or/adc/sbb/and/sub/xor/cmp rm16/32/64, imm8
  // - 0x83
    ir_emit_group1_imm(INSTS, inst);
}

IR_HANDLE(test_rm_reg)
{ // test rm8, r8 - 0x84
    x86_size_e size_e = inst->operand_reg.size;
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
...
IR_HANDLE(mov_rm_reg)
{ // mov rm8/16/32/64, r8/16/32/64 - 0x88
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
    ir_emit_set_rm(INSTS, &inst->operand_rm, reg);
}

IR_HANDLE(mov_reg_rm)
{ // mov r8/16/32/64, rm8/16/32/64 - 0x8a
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_emit_set_reg(INSTS, &inst->operand_reg, rm);
}

IR_HANDLE(mov_r32_rm32)
{ // mov r16/32/64, rm16/32/64 - 0x8b
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_emit_set_reg(INSTS, &inst->operand_reg, rm);
}
...
{ // stosd - 0xab
    ...
    ir_instruction_t* rdi_add =
        ir_emit_add(INSTS, rdi, ir_emit_immediate(INSTS, get_bit_size(size_e) / 8));
...
IR_HANDLE(group2_rm8_imm8)
{ // rol/ror/rcl/rcr/shl/shr/sal/sar rm8, imm8 - 0xc0
    ir_emit_group2(INSTS, inst, ir_emit_immediate(INSTS, inst->operand_imm.immediate.data));
}

IR_HANDLE(group2_rm32_imm8)
{ // rol/ror/rcl/rcr/shl/shr/sal/sar rm16/32/64,
  // imm8 - 0xc1
    ir_emit_group2(INSTS, inst, ir_emit_immediate(INSTS, inst->operand_imm.immediate.data));
}
...
{ // mov rm16/32/64, imm16/32/64 - 0xc7
    ir_instruction_t* imm = ir_emit_immediate_sext(INSTS, &inst->operand_imm);
    ir_emit_set_rm(INSTS, &inst->operand_rm, imm);
}
...
{ // leave - 0xc9
    ...
    ir_instruction_t* popped_value =
        size == X86_SIZE_WORD ? ir_emit_read_word(INSTS, rbp) : ir_emit_read_qword(INSTS, rbp);
...
IR_HANDLE(group2_rm8_1)
{ // rol/ror/rcl/rcr/shl/shr/sal/sar rm8, 1 - 0xd0
    ir_emit_group2(INSTS, inst, ir_emit_immediate(INSTS, 1));
}

IR_HANDLE(group2_rm32_1)
{ // rol/ror/rcl/rcr/shl/shr/sal/sar rm16/32/64, 1 - 0xd1
    ir_emit_group2(INSTS, inst, ir_emit_immediate(INSTS, 1));
}

IR_HANDLE(group2_rm32_cl)
{ // rol/ror/rcl/rcr/shl/shr/sal/sar rm16/32/64, cl - 0xd3
...
IR_HANDLE(group3_rm8)
{ // test/not/neg/mul/imul/div/idiv rm8, imm8 - 0xf6
    ir_emit_group3(INSTS, inst);
}

IR_HANDLE(group3_rm32)
{ // test/not/neg/mul/imul/div/idiv rm16/32/64, imm32 - 0xf7
    ir_emit_group3(INSTS, inst);
}
...
{ // inc/dec/call/jmp/push rm32
    ...
    switch (opcode)
    {
        ...
        default:
        {
            ERROR("Unimplemented group 5 opcode: %02x during %016lx", opcode,
                  state->current_address - g_base_address);
            break;
        }
    }
}
...
{ // group 7 - 0x0f 0x01
    ...
    switch (modrm.reg)
    {
        ...
        default:
        {
            ERROR("Unimplemented group 7 opcode: %02x during %016lx", opcode,
                  state->current_address - g_base_address);
            break;
        }
    }
...
IR_HANDLE(syscall)
{ // syscall - 0x0f 0x05
    x86_ref_e inputs[] = {X86_REF_RAX, X86_REF_RDI, X86_REF_RSI, X86_REF_RDX,
                          X86_REF_R10, X86_REF_R8,  X86_REF_R9};
...
{ // movups/movaps xmm, xmm128 - 0x0f 0x11
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_emit_set_reg(INSTS, &inst->operand_reg, rm);
}
...
IR_HANDLE(movhps_xmm_m64)
{
    ...
    ir_instruction_t* xmm_dest =
        ir_emit_insert_integer_to_vector(INSTS, xmm, m64, 1, X86_SIZE_QWORD);
    ir_emit_set_reg(INSTS, &inst->operand_reg, xmm_dest);
}

IR_HANDLE(mov_xmm128_xmm)
{ // movups/movaps xmm128, xmm - 0x0f 0x29
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
    ir_emit_set_rm(INSTS, &inst->operand_rm, reg);
}
...
IR_HANDLE(movq_mm_rm32)
{ // movq mm, rm32 - 0x0f 0x6e
    ERROR("Unimplemented instruction: movq mm, rm32 - 0x0f 0x6e during %016lx",
          state->current_address - g_base_address);
}
...
IR_HANDLE(imul_r32_rm32)
{ // imul r32/64, rm32/64 - 0x0f 0xaf
    x86_size_e size_e = inst->operand_reg.size;
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
    ir_instruction_t* result =
        ir_emit_imul(INSTS, ir_emit_sext(INSTS, reg, size_e), ir_emit_sext(INSTS, rm, size_e));
    ir_emit_set_reg(INSTS, &inst->operand_reg, result);
}
...
IR_HANDLE(movzx_r32_rm8)
{ // movzx r32/64, rm8 - 0x0f 0xb6
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_emit_set_gpr64(INSTS, inst->operand_reg.reg.ref, rm);
}

IR_HANDLE(movzx_r32_rm16)
{ // movzx r32/64, rm16 - 0x0f 0xb7
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_emit_set_gpr64(INSTS, inst->operand_reg.reg.ref, rm);
}
...
IR_HANDLE(punpcklbw_xmm_xmm128)
{ // punpcklbw xmm, xmm/m128 - 0x66 0x0f 0x60
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
...
IR_HANDLE(punpcklwd_xmm_xmm128)
{ // punpcklwd xmm, xmm/m128 - 0x66 0x0f 0x61
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
...
IR_HANDLE(punpckldq_xmm_xmm128)
{ // punpckldq xmm, xmm/m128 - 0x66 0x0f 0x62
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
...
IR_HANDLE(group14_xmm)
{ // group14 xmm - 0x66 0x0f 0x73
    ...
    switch (opcode)
    {
        ...
        default:
        {
            ERROR("Unimplemented group 14 opcode: %02x during %016lx", opcode,
                  state->current_address - g_base_address);
            break;
        }
    }
...
IR_HANDLE(punpcklqdq_xmm_xmm128)
{ // punpcklqdq xmm, xmm/m128 - 0x66 0x0f 0x6c
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
...
{ // movq xmm, rm32 - 0x66 0x0f 0x6e
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* vector = ir_emit_vector_from_integer(INSTS, rm);
    ir_emit_set_reg(INSTS, &inst->operand_reg, vector);
}

IR_HANDLE(movdqa_xmm_xmm128)
{ // movdqa xmm, xmm128 - 0x66 0x0f 0x6f
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_emit_set_reg(INSTS, &inst->operand_reg, rm);
}

IR_HANDLE(pshufd_xmm_xmm128_cb)
{ // pshufd xmm, xmm/m128, imm8 - 0x66 0x0f 0x70
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* result =
        ir_emit_vector_packed_shuffle_dword(INSTS, rm, inst->operand_imm.immediate.data);
    ir_emit_set_reg(INSTS, &inst->operand_reg, result);
}

IR_HANDLE(pcmpeqb_xmm_xmm128)
{ // pcmpeqb xmm, xmm/m128 - 0x66 0x0f 0x74
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
...
IR_HANDLE(pcmpeqw_xmm_xmm128)
{ // pcmpeqw xmm, xmm/m128 - 0x66 0x0f 0x75
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
...
IR_HANDLE(pcmpeqd_xmm_xmm128)
{ // pcmpeqd xmm, xmm/m128 - 0x66 0x0f 0x76
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
...
IR_HANDLE(movq_rm32_xmm)
{ // movq rm32, xmm - 0x66 0x0f 0x7e
    ir_instruction_t* xmm = ir_emit_get_reg(INSTS, &inst->operand_reg);
    ir_instruction_t* rm = ir_emit_integer_from_vector(INSTS, xmm);
    ir_emit_set_rm(INSTS, &inst->operand_rm, rm);
}

IR_HANDLE(paddq_xmm_xmm128)
{ // paddq xmm, xmm/m128 - 0x66 0x0f 0xd4
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
...
IR_HANDLE(movq_xmm64_xmm)
{ // movq xmm64, xmm - 0x66 0x0f 0xd6
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
    if (inst->operand_rm.type == X86_OP_TYPE_MEMORY)
    {
...
IR_HANDLE(pmovmskb_reg_xmm)
{ // pmovmskb reg, xmm - 0x66 0x0f 0xd7
    ir_instruction_t* xmm = ir_emit_get_reg(INSTS, &inst->operand_reg);
    ir_instruction_t* result = ir_emit_vector_packed_move_byte_mask(INSTS, xmm);
    ir_emit_set_reg(INSTS, &inst->operand_rm, result);
}

IR_HANDLE(pminub_xmm_xmm128)
{ // pminub xmm, xmm/m128 - 0x66 0x0f 0xda
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
...
IR_HANDLE(pand_xmm_xmm128)
{ // pand xmm, xmm/m128 - 0x66 0x0f 0xdb
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
...
IR_HANDLE(por_xmm_xmm128)
{ // por xmm, xmm/m128 - 0x66 0x0f 0xeb
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
...
IR_HANDLE(pxor_xmm_xmm128)
{ // pxor xmm, xmm/m128 - 0x66 0x0f 0xef
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
...
IR_HANDLE(psubb_xmm_xmm128)
{ // psubb xmm, xmm/m128 - 0x66 0x0f 0xf8
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* reg = ir_emit_get_reg(INSTS, &inst->operand_reg);
...
IR_HANDLE(movdqu_xmm_xmm128)
{ // movdqu xmm, xmm128 - 0xf3 0x0f 0x6f
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_emit_set_reg(INSTS, &inst->operand_reg, rm);
}

IR_HANDLE(movq_xmm_xmm64)
{ // movq xmm, xmm64 - 0xf3 0x0f 0x7e
    x86_operand_t rm_op = inst->operand_rm;
...
IR_HANDLE(pcmpistri_xmm_xmm128_cb)
{ // pcmpistri xmm, xmm/m128, imm8 - 0x66
  // 0x0f 0x3a 0x63
    ir_instruction_t* rm = ir_emit_get_rm(INSTS, &inst->operand_rm);
    ir_instruction_t* imm = ir_emit_immediate(INSTS, inst->operand_imm.immediate.data);
