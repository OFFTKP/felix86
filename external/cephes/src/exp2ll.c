...
float128_t cephes_f128_exp2l(float128_t x) {
    ...
    px = cephes_f128_floorl(f128_add(x, f_0_p5));
    n = f128_to_i32(px, softfloat_round_near_even, true);
    x = f128_sub(x, px);
     ...
     */
    xx = f128_mul(x, x);
    px = f128_mul(x, cephes_f128_polevll(xx, P, 4));
    x = f128_div(px, f128_sub(cephes_f128_p1evll(xx, Q, 5), px));
    x = f128_add(one, cephes_f128_ldexpl(x, 1));
