...
*/
static float128_t DP1 = {0x8400000000000000ULL, 0x3ffe921fb54442d1ULL};
// 7.853981633974483067550664827649598009884357452392578125E-1L;
static float128_t DP2 = {0xe000000000000000ULL, 0x3fc4a62633145c06ULL};
// 2.8605943630549158983813312792950660807511260829685741796657E-18L;
static float128_t DP3 = {0xa67cc74020bbea64ULL, 0x3f8bcd129024e088ULL};
...
float128_t cephes_f128_sinl(float128_t x) {
    ...
    y = cephes_f128_floorl(f128_div(x, F128_PIO4L)); /* integer part of x/PIO4 */

    /* strip high bits of integer part to prevent integer overflow */
    z = cephes_f128_ldexpl(y, -4);
    z = cephes_f128_floorl(z);                 /* integer part of y/8 */
    z = f128_sub(y, cephes_f128_ldexpl(z, 4)); /* y - 16 * (y/16) */

    j = f128_to_i32(z, softfloat_round_near_even, true); /* convert to integer for tests on the phase angle */
    /* map zeros to origin */
    if (j & 1) {
        j += 1;
        y = f128_add(y, one);
    ...
    {
        float128_t tmp1 = f128_mul(y, DP1);
        float128_t tmp2 = f128_mul(y, DP2);
        float128_t tmp3 = f128_mul(y, DP3);
        float128_t tmp4 = f128_sub(x, tmp1);
        float128_t tmp5 = f128_sub(tmp4, tmp2);
        z = f128_sub(tmp5, tmp3);
    }

    z = f128_sub(f128_sub(f128_sub(x, f128_mul(y, DP1)), f128_mul(y, DP2)), f128_mul(y, DP3));

    zz = f128_mul(z, z);
    if ((j == 1) || (j == 2)) {
        // y = 1.0L - ldexpl(zz,-1) + zz * zz * polevll( zz, coscof, 10 );
        float128_t tmp1 = f128_mul(zz, zz);
        float128_t tmp2 = f128_mul(tmp1, cephes_f128_polevll(zz, coscof, 10));
        float128_t tmp3 = f128_sub(one, cephes_f128_ldexpl(zz, -1));
        y = f128_add(tmp3, tmp2);
    } else {
        // y = z  +  z * (zz * polevll( zz, sincof, 11 ));
        float128_t tmp1 = f128_mul(zz, cephes_f128_polevll(zz, sincof, 11));
        float128_t tmp2 = f128_mul(z, tmp1);
        y = f128_add(z, tmp2);
    }
...
float128_t cephes_f128_cosl(float128_t x) {
    ...
    y = cephes_f128_floorl(f128_div(x, F128_PIO4L));
    z = cephes_f128_ldexpl(y, -4);
    z = cephes_f128_floorl(z);                 /* integer part of y/8 */
    z = f128_sub(y, cephes_f128_ldexpl(z, 4)); /* y - 16 * (y/16) */

    /* integer and fractional part modulo one octant */
    i = f128_to_i32(z, softfloat_round_near_even, true);
