...
float128_t cephes_f128_atanl(float128_t x) {
    ...
    if (f128_lt(T3P8, x)) {
        y = F128_PIO2L;
        x = f128_complement_sign(f128_div(one, x));
    }

    else if (f128_lt(TP8, x)) {
        y = F128_PIO4L;
        x = f128_div(f128_sub(x, one), f128_add(x, one));
    } else
        y = zero;

    /* rational form in x**2 */
    z = f128_mul(x, x);
    y = f128_add(f128_add(y, f128_mul(f128_mul(f128_div(cephes_f128_polevll(z, P, 8), cephes_f128_p1evll(z, Q, 8)), z), x)), x);
...
#endif
{
...
#else
            return (f128_mul(f_3_p0, F128_PIO2L));
#endif
        }
        if (f128_eq(y, zero))
            return zero;
    ...
    case 2:
        ...
        break;
    case 3:
        w = f128_complement_sign(F128_PIL);
        break;
#else
    case 0:
        w = zero;
    ...
    z = cephes_f128_atanl(f128_div(y, x));
