...
static float128_t tancotl(float128_t xx, int cotflg);
...
static float128_t tancotl(float128_t xx, int cotflg) {
    ...
    j = f128_to_i32(z, softfloat_round_near_even, true);
    ...
    // if( zz > 1.0e-20L )
    if (f128_lt(max_quad, zz)) {
        y = f128_add(z, f128_mul(z, f128_div(f128_mul(zz, cephes_f128_polevll(zz, P, 5)), cephes_f128_p1evll(zz, Q, 6))));
    } else {
        y = z;
    }
