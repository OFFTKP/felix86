...
float128_t cephes_f128_log2l(float128_t x) {
     ...
     */
    if ((e > 2) || (e < -2)) {
        if (f128_lt(x, SQRTH)) { /* 2( 2x-1 )/( 2x+1 ) */
            e -= 1;
            z = f128_sub(x, f_0_p5);
            y = f128_add(f128_mul(f_0_p5, z), f_0_p5);
        } else { /*  2 (x-1)/(x+1)   */
            z = f128_sub(x, f_0_p5);
            z = f128_sub(z, f_0_p5);
            y = f128_add(f128_mul(f_0_p5, x), f_0_p5);
        }
        x = f128_div(z, y);
        z = f128_mul(x, x);
        y = f128_mul(x, f128_div(f128_mul(z, cephes_f128_polevll(z, R, 5)), cephes_f128_p1evll(z, S, 6)));
        goto done;
    }
    ...
    if (f128_lt(x, SQRTH)) {
        e -= 1;
        x = f128_sub(cephes_f128_ldexpl(x, 1), one); /*  2x - 1  */
    } else {
        x = f128_sub(x, one);
    }
    z = f128_mul(x, x);
    y = f128_mul(x, f128_div(f128_mul(z, cephes_f128_polevll(x, P, 12)), cephes_f128_p1evll(x, Q, 12)));
    y = f128_sub(y, cephes_f128_ldexpl(z, -1)); /* -0.5x^2 + ... */
     ...
     */
    z = f128_mul(y, LOG2EA);
    z = f128_add(z, f128_mul(x, LOG2EA));
    z = f128_add(z, y);
    z = f128_add(z, x);
    z = f128_add(z, i32_to_f128(e));
    return (z);
}
